...
class SharedEnvData():
    ...
    def fill_actions(self, action_batch):
        ...
        if th.cuda.is_initialized() and self._shared_data._device != action_batch:
            if action_batch.device.type == "cuda": # synchronize always unless cuda has never even been used (this way we save a lot of memory)
                th.cuda.synchronize(action_batch.device.type)
