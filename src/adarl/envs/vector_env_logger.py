    ...
    def step(
        ...
        if completed_eps_count>0:
            ...
            if self._log_infos:
                ...
                #     final_infos = infos["final_info"]
                #     final_infos = {k:v for k,v in final_infos.items() if k != "final_info"} # make a shallow copy without the final_info cycle
                #     # completed_final_infos = map_tensor_tree(final_infos, lambda l: l[completed_eps])
                #     # completed_final_infos = flatten_tensor_tree(completed_final_infos)
                #     # completed_final_infos = {"VecEnvLogger/lastinfo."+(".".join(k)):v for k,v in completed_final_infos.items()} # convert keys to strings
                #     # completed_final_infos = map_tensor_tree(completed_final_infos, lambda l: int(l) if isinstance(l, bool) else l)
                #     # completed_final_infos = map_tensor_tree(completed_final_infos,
