...
class VectorEnvLogger(
    ...
    def __init__(
        ...
        self._ep_rewards_sl = th.zeros(size=(self._num_envs,), device=self._env_th_device)
        self._ep_durations_sl = th.zeros(size=(self._num_envs,), device=self._env_th_device, dtype=th.long)
    ...
    def step(
        ...
        self._ep_rewards_sl += reward
        self._ep_durations_sl += 1
        ...
        if completed_eps_count>0:
            completed_rews = self._ep_rewards_sl[completed_eps]
            completed_durs = self._ep_durations_sl[completed_eps]
            ...
            self._completed_ep_durations_min_sl = th.min(th.min(completed_durs), self._completed_ep_rewards_min_sl)
            self._completed_ep_durations_max_sl = th.max(th.max(completed_durs), self._completed_ep_rewards_max_sl)
            self._completed_ep_count_sl += completed_eps_count
            if self._completed_ep_count_sl >= self.num_envs:
                ...
                self._completed_ep_count_sl = 0
            self._ep_rewards_sl[completed_eps] = 0.0
            self._ep_durations_sl[completed_eps] = 0
            self._completed_ep_count_sl = 0
